...
import { LineChart, BarChart, PieChart, Lock } from "lucide-react";
import {
...
// Helper component for blurred/locked analytics
function BlurredLockOverlay({ children, feature, onUpgrade }) {
  return (
    <div className="relative">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center z-10">
        <div className="flex flex-col items-center justify-center bg-white/70 rounded-lg p-4">
          <Lock className="h-8 w-8 text-gray-500 mb-2" />
          <div className="font-semibold text-gray-700 text-center mb-1">{feature} Locked</div>
          <div className="text-gray-500 text-sm mb-2 text-center">Upgrade to Enterprise to unlock {feature}.</div>
          <button
            className="bg-gradient-to-r from-blue-600 to-purple-500 text-white px-4 py-2 rounded font-semibold shadow hover:from-blue-700 hover:to-purple-600 transition"
            onClick={onUpgrade}
          >
            Upgrade Now
          </button>
...
export default function AnalyticsClientPage() {
  ...
  // Helper: For Professional tier, only allow these analytics
  const isMiddleTier = currentTier === 'middle';
  const showTopRevenueClients = isMiddleTier || isAnalyticsFeatureEnabled('topRevenueClients');
  const showRecentPayments = isMiddleTier || isAnalyticsFeatureEnabled('recentPayments');
  const showRecentSessions = isMiddleTier || isAnalyticsFeatureEnabled('recentSessions');
  ...
  return (
    <div className="flex-1">
      ...
      <main className="p-6 space-y-8">
        {/* Top grid: All analytics except the three allowed for middle tier */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
          {/* Revenue Trend */}
          {isMiddleTier ? (
            <Card>
              <BlurredLockOverlay feature="Revenue Analytics" onUpgrade={handleUpgrade}>
                <CardHeader>
                    ...
                    <div className="h-40">
                      <ResponsiveContainer width="100%" height="100%">
                        <RechartsLineChart data={revenueData}>
                          ...
                          <XAxis dataKey="day" fontSize={12} tick={{ fontSize: 10 }} />
                          <YAxis fontSize={12} tick={{ fontSize: 10 }} tickFormatter={(value) => `$${value}`} />
                          <Tooltip formatter={(value: number) => [`$${value}`, "Revenue"]} labelFormatter={(label) => `Month: ${label}`} />
                          <Line type="monotone" dataKey="revenue" stroke="#2563eb" strokeWidth={2} dot={{ fill: "#2563eb", strokeWidth: 2, r: 4 }} activeDot={{ r: 6, stroke: "#2563eb", strokeWidth: 2 }} />
                        </RechartsLineChart>
                      </ResponsiveContainer>
                    </div>
                  )}
          ...
          {isMiddleTier ? (
            <Card>
              <BlurredLockOverlay feature="Client Growth Analytics" onUpgrade={handleUpgrade}>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
          ...
          {isMiddleTier ? (
            <Card>
              <BlurredLockOverlay feature="Session Analytics" onUpgrade={handleUpgrade}>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
          ...
          {isMiddleTier ? (
            <Card>
              <BlurredLockOverlay feature="Session Time Analytics" onUpgrade={handleUpgrade}>
                <CardHeader>
        ...
        {/* Bottom grid: Only for middle and highest tiers, but order changes for middle */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
          {/* For middle tier: Top Revenue Clients, Recent Payments, Recent Sessions */}
          {isMiddleTier && (
            <>
